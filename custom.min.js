...
mSearch2.setFilters(params);mSearch2.load(params);}});},1000);this.initialized=true;},handlePagination:function(){var pcre=new RegExp(mse2Config['pageVar']+'[=|\/|-](\\d+)');console.log('pcre ',pcre);switch(mse2Config['mode']){case 'button':this.pagination.hide();this.results.after(this.options['more_tpl']);var more=$(this.options['more']);var has_results=false;$(this.options['pagination_link']).each(function(){var href=$(this).prop('href');var match=href.match(pcre);var page=!match?1:match[1];if(page>mse2Config['page']){has_results=true;return false;}});if(!has_results){more.hide();}
if(mse2Config['page']>1){mse2Config['page']='';mSearch2.Hash.remove(mse2Config.pageVar);mSearch2.load();}
$(document).on('click',this.options['wrapper']+' '+this.options['more'],function(e){e.preventDefault();mSearch2.addPage();});break;case 'scroll':this.pagination.hide();var wrapper=$(this.options['wrapper']);var $window=$(window);$window.on('scroll',function(){if(!mSearch2.reached&&$window.scrollTop()>wrapper.height()-$window.height()){mSearch2.reached=true;mSearch2.addPage();}});if(mse2Config['page']>1){mse2Config['page']='';mSearch2.Hash.remove(mse2Config.pageVar);mSearch2.load();}
...
mse2Config['sort']=(sort!=mse2Config['start_sort'])?sort:'';var params=mSearch2.getFilters();if(mse2Config['page']>1&&(mse2Config['mode']=='scroll'||mse2Config['mode']=='button')){mse2Config['page']='';delete(params[mse2Config.pageVar]);}
mSearch2.Hash.set(params);mSearch2.load(params);return false;});},handleTpl:function(){$(document).on('click',this.options.tpl_link,function(){if(!$(this).hasClass(mSearch2.options.active_class)){$(mSearch2.options.tpl_link).removeClass(mSearch2.options.active_class);$(this).addClass(mSearch2.options.active_class);var tpl=$(this).data('tpl');mse2Config['tpl']=(tpl!=mse2Config['start_tpl']&&tpl!=0)?tpl:'';var params=mSearch2.getFilters();if(mse2Config['page']>1&&(mse2Config['mode']=='scroll'||mse2Config['mode']=='button')){mse2Config['page']='';delete(params[mse2Config.pageVar]);}
...
var params=mSearch2.getFilters();if(mse2Config['page']>1&&(mse2Config['mode']=='scroll'||mse2Config['mode']=='button')){mse2Config['page']='';delete(params[mse2Config.pageVar]);}
...
if(mse2Config['page']>0){params[mse2Config.pageVar]=mse2Config['page'];}
if(mse2Config['limit']>0&&mse2Config['limit']!=mse2Config['start_limit']){params.limit=mse2Config['limit'];}
...
break;case 'pagination':mse2Config['page']=params[mse2Config.pageVar]==undefined
?''
:params[mse2Config.pageVar];break;case 'sort':var sorts={};values=params['sort'];if(values==undefined){values=mse2Config['start_sort'];mse2Config['sort']='';}
...
else{item.data('dir','').attr('data-dir','');item.removeClass(mSearch2.options['active_class']);}});break;case 'tpl':values=params['tpl']!=undefined
?params['tpl']
:0;$(document).find(this.options['tpl_link']).each(function(){item=$(this);if(item.data('tpl')==values){item.addClass(mSearch2.options['active_class']);mse2Config['tpl']=values;}
...
break;default:if(mSearch2.sliders[name]){if(item.prop('id').match(/.*?_0$/)){val=(values!=undefined&&values[0]!=undefined)
?values[0]
:mSearch2.sliders[name]['values'][0];}
else{val=(values!=undefined&&values[1]!=undefined)
?values[1]
...
break;case 'OPTION':var text=input.text();var matches=text.match(/\([^\)]+\)$/);var src=matches
?matches[0]
:'';var dst='';if(!count){input.prop('disabled',true).addClass(mSearch2.options.disabled_class);}
...
delete(params[mse2Config.pageVar]);var action=$(this.options.filters).attr('action');if(!mSearch2.options.autoLoad){var vars='';for(var i in params){if(params.hasOwnProperty(i)){vars+='&'+i+'='+params[i];}}
